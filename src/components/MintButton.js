...
function MintButton({ web3 }) {
  ...
  useEffect(() => {
    if (!web3) {
      ...
      const contract = new web3.eth.Contract(Justcubes.abi, contractAddress)
  ...
  const mintToken = async () => {
    if (!web3) {
      ...
      const contract = new web3.eth.Contract(Justcubes.abi, Justcubes.address)
    ...
    <div>
      <div className="flex flex-col md:flex-row mt-12 md:space-x-12 md:space-y-0 space-y-12" style={{ display: "flex", backgroundColor: "transparent", paddingBottom: "20px" }}>
        ...
        <div className={styles.showItemDiv}>
          ...
          <div className={styles.showItemInfo}>0.05 ETH</div>
        </div>
      </div>
